/*
  ...
  1. New Data
    - Sample teams, profiles, and team memberships
    - Security scanners, pipelines, and pipeline stages
    - Infrastructure nodes and integrations
    - Sample vulnerabilities, alerts, and system events
  
  2. Changes
    - Removed direct references to non-existent auth.users
    - Set user_id to NULL for sample profiles
    - Added ON CONFLICT clauses to prevent duplicate entries
*/
...
-- Insert sample profiles (with NULL user_id to avoid foreign key constraint)
INSERT INTO profiles (id, user_id, email, full_name, role, department, preferences) VALUES
  ('650e8400-e29b-41d4-a716-446655440001', NULL, 'admin@company.com', 'System Administrator', 'admin', 'IT Operations', '{"theme": "dark", "notifications": true}'),
  ('650e8400-e29b-41d4-a716-446655440002', NULL, 'devops@company.com', 'DevOps Engineer', 'devops', 'Platform Engineering', '{"theme": "light", "notifications": true}'),
  ('650e8400-e29b-41d4-a716-446655440003', NULL, 'security@company.com', 'Security Analyst', 'security', 'Security Operations', '{"theme": "dark", "notifications": true}')
